...
def results(request):
	...
	for usn in tqdm(l,desc = "Scraping",unit = 'data'):
		...
		if captha_check and 'Invalid captcha' in page_soup.script.text.strip():
			captha_check = False
		...
		for con in range(1,len(container)-1):
			...
			if marks[0].text == 'Subject Code':
					break
			if debug : print(marks[0].text ,marks[2].text+ ',' + marks[3].text + ',' +  marks[4].text + ',', marks[5].text )

			# Try catch if Key is not found in Dict

			try:
				data[(marks[0].text,"Internal")][Usn] = int(marks[2].text)
			except(KeyError):
			...
			try:
				data[(marks[0].text,"External")][Usn] = int(marks[3].text)
			except(KeyError):
			...
			try:
				data[(marks[0].text,"Total")][Usn] = int(marks[4].text)
			except(KeyError):
	...
	pass_ia_marks = int(request.POST['passing_ia'])
	mydb = connect_db()
	conn = mydb.cursor()
